...
`@citisen/dsh-sentry` is published in two halves:
...
- `npm run check` fails — `lib/client.js` does not match `src/client.js`, or the
  envelope, the sentry engine, the favicon builder, or the settings row is broken
...
```sh
npm view @citisen/dsh-sentry --json | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);const v=j['dist-tags'].latest;console.log(v, j.versions[v].dist.integrity);console.log(j.versions[v].dist.attestations)})"
```
