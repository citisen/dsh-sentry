...
用 Web Audio 合成 —— 不打包任何音频文件，也不联网取任何东西。**音符是配置文档里的字面量**，不再是插件里写死的一张表：

```
volume 0.5                              # 默认音量：没在自己块里写 volume 的状态用它
waiting  { chime A5 E6 }                # 上扬的双音 → 50%
approval { chime A5     volume 0.45 }   # 单音 → 45%
done     { chime A4     volume 0.25 }   # 低八度，明显更轻 → 25%
```

音名（`A5`、`E6`、`C#4`、`Bb3`）或者频率（`880 1318.5`）都可以，按写下的顺序播放：一个音 130ms，后一个比前一个晚 90ms 起音，所以两个音是叠着出来的，听上去是一个音程而不是两声敲门。频率走等程律，A4 = 440Hz —— 也就是说 `A5 E6` 复现的正是原来那对音，只是换了一种看得懂的写法。`chime off` 让这个状态单独闭嘴。

`volume` 在两个地方出现，但**只有一个含义：音量**。顶层那行是默认值，块里那行是覆盖值 —— 块里的**替换**它，不与它相乘。所以预览卡片上印的百分比，永远等于文档里某一行的数字（顶层那行，或者这个状态自己那行）；两边都没写时用出厂 0.5，卡片会标「（默认）」。这一条是被改掉的设计：早先版本顶层是"总音量"、块里是"倍数"，而**每状态的倍数还写死在插件里** —— 于是你写 `volume 1`，卡片上却出现 85% 和 45%，文档里任何地方都找不到这两个数。

| 状态 | 出厂声音 | 为什么 |
| --- | --- | --- |
| 等待回答 | `A5 E6`（默认音量 0.5） | 唯一表示"需要你立刻做决定"的声音，所以它是唯一上扬的双音 |
| 等待审批 | `A5`，`volume 0.45` | 单音 —— 是另一件事，但不是另一件急事 |
| 刚刚完成 | `A4`，`volume 0.25` | 低一个八度且明显更轻：完成属于背景信息 |
| 执行中 | 不发声 | 一轮开始不是这个插件会打扰你的事件；`running` 块里写 `chime` 会被报成"写了但没有用"，而不是被悄悄忽略 |
...
编辑器随文档长高，不给自己加滚动条：页面本来就在滚，一份文档两条滚动条只会让人抓错那一根。留下来的上限是为了别让一份几百行的粘贴把设置页拉成一条长线，而不是一个显示决策 —— 出厂的文档（42 行）一定完整可见。
...
出厂的文档（也是"全部恢复默认"回到的那一份）就是四个状态各一段、音名写成音名：
...
| `verify-client.mjs` | Node + 桩 | 全部纯决策：状态优先级、完成沿、图标几何、标题合成、声音门控，**文档读取器**（块语法、时长与音名、每一类报错的行号与回退）、**编辑器的语法**（着色、诊断范围、补全列表、词汇表确实来自插件的常量而不是自带一份），以及设置行（四张预览卡片画的确实是标签页会画的那张图、试听按钮播的确实是卡片上印的那一声、问题列表带行号），最后是 `apply()` 对桩服务的端到端接线 |
...
- **`running` 块里的 `chime`/`volume` 会被报成"没有用"。** 这不是错误而是警告：写成 `chime off` 也好、删掉也好，它都不会响，因为"一轮开始"不是这个插件会打扰你的事件。让它在文档里是合法的，比让读取器假装没看见要好。
