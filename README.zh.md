...
| 等待回答时提示 | 开 | 上扬的双音 |
| 等待审批时提示 | 开 | 单音 |
| 会话完成时提示 | **关** | 轻低音 —— 默认关，因为"完成"属于背景信息，每轮都响一声正是人们最后把提示音全关掉的原因 |
...
- **"刚刚完成"需要一个衰减窗口才有意义。** 把*完成状态保留时间*设为 0 就永远不会出现绿色信号，这是一种合法配置而不是故障。
