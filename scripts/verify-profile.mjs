 ...
 */
const { ALERT_NAMESPACE: ROW_ID } = await import(
...
// The services this plugin injects are not declared in `dsh.client.inject`
// (that field is for other client bundles), so nothing is checked here beyond
// the self-reference guard — the runtime contract is asserted by
// `verify-client.mjs`, which reads the exported `inject` array.
