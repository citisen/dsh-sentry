  ...
  ctx.inject(['settings'], (settingsCtx) => {
    if (typeof settingsCtx.settings.register !== 'function') {
      settingsCtx.logger.error(
        `dsh-sentry: this dsh has no settings.register() (0.1.7-alpha.1 replaced the settings API), ` +
          `so the durable "${ALERT_NAMESPACE}" section is not registered and the sentry cannot be ` +
