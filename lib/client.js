...
function reportMissingSettings() {
  ...
  console.error(
    `${PLUGIN_ID}: dsh is not providing the "${SETTINGS_SERVICE}" service, so the alert document ` +
      `cannot be read or saved and the sentry runs on its shipped defaults. dsh 0.1.7-alpha.1 ` +
      `replaced it with "${REPLACEMENT_SETTINGS_SERVICE}"; this build targets the dsh 0.1.5-rc.x ` +
      'line (latest/next). Pin dsh to 0.1.5-rc.x, or install a newer @citisen/dsh-sentry.',
  )
}
 ...
 * composition this build does not recognize still gets the sentry, the row, and a
...
function apply(ctx) {
  ...
  // while it stays: a replacement or an unload puts the sentry back on defaults.
  ctx.inject([SETTINGS_SERVICE], (settingsCtx) => {
    const boundScope = settingsCtx[SETTINGS_SERVICE].bind({
    ...
    scope = boundScope
    settingsCtx.effect(() => boundScope.subscribe(adopt), 'dsh-sentry: settings adoption')
    adopt()
    return () => {
